...
### [技能的使用](https://code.claude.com/docs/en/skills)

与 `CLAUDE.md` 内容不同，技能的内容只有在被使用时才会加载，因此，在你需要它之​​前，长期参考资料几乎不需要任何成本。

保持正文简洁。技能加载完毕后，其内容会在回合间保持上下文一致，因此每一行都会消耗一个代币。直接说明要做什么，而不是叙述如何做或为什么做，并采用与CLAUDE.md 内容相同的简洁性测试方法。

添加disable-model-invocation: true此设置可阻止 Claude 自动触发任务。

### [自定义状态栏](https://code.claude.com/docs/en/statusline)

使得每次打开 Claude Code 都能看到特定的信息，例如当前使用的分支、模型、开销等。

不消耗 token， 只是拦截消息传递，并把参数展示出来。

> 注：只能定义一个状态栏脚本，展示多个信息的话可以写一个脚本在脚本里多行格式化展示。
