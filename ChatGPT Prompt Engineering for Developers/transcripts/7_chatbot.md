...
[0:42] get into it. So first we'll set up the OpenAI Python package as
[0:46] usual.
...
[0:56] And so although the chat format is designed to make multi-turn
...
[1:10] helper functions. So this is the one that we've been using throughout all the
...
[2:01] So here's an example of a list of messages and so the first
...
[2:19] messages are the user messages and then ChatGPT's
...
[7:38] This way the model has the information it needs to
...
[7:54] And note that every time we call the language model, we're
...
[9:26] for an address. Finally, you collect the payment. Make sure to
...
[10:24] we kind of asked the assistant in the
