...
[0:56] to use another one of the model's input parameters called
...
[2:00] And so here the instruction is "You are a customer service AI assistant.
[2:04] Your task is to send an email reply to a valued customer.
...
[2:29] And when you're using a language model to
...
[3:13] out to customer service because this is just
...
[3:29] can kind of think of temperature as the
...
[4:01] at an even higher temperature, it might even choose "tacos", which only
...
[5:10] specified a model and then also a temperature, but we've
